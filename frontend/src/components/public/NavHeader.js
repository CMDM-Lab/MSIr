            ...
            <div class="container-fluid">
                ...
                <div className="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul className="navbar-nav me-auto mb-2 mb-lg-0">
                        ...
                        <li className={`nav-item ${currentState==='home'?'active':''}`}>
                            <p ><NavLink to='/home' style={{ color:"#000000" }}
                            onClick={()=>{setCurrentState('home');}}>Home</NavLink></p>
                        </li>
                        ...
                        {Boolean(userDetails.token)?(
                            <li className={`nav-item ${currentState==='dataset'?'active':''}`}>
                                <p ><NavLink to='/datasets' style={{ color:"#000000"} }
                                onClick={()=>setCurrentState('dataset')}>Dataset</NavLink></p>
                            </li>):''
                        }
                        <li className={`nav-item ${currentState==='Document'?'active':''}`}>
                            <p ><NavLink to='/doc' style={{ color:"#000000"}}
                            onClick={()=>setCurrentState('Document')}>Docs</NavLink></p>
                        </li>
              ...
              {Boolean(userDetails.token)?(
                ...
                <li className={`nav-item ${currentState==='Sign in'?'active':''}`}>
                    <p ><NavLink to='/users/sign_in' style={{ color:"#000000"}}
                    onClick={()=>setCurrentState('Sign in')}>Sign in</NavLink></p>
                </li>)
                }
            </ul>
          </div>
